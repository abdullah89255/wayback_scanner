...
def sanitize_filename(url: str, timestamp: str) -> str:
    ...
    # Preserve the real extension (.js/.json/.env/.sql etc.) so saved files
    # open correctly. Fall back to .html only when no known extension found.
...
def fetch_snapshots(url: str, years: int, session: requests.Session) -> list:
    """
    ...
    params = {
        ...
        "from":     from_date,
        "filter":   "!statuscode:404",  # exclude 404s (correct CDX negation: prefix before field)
        "limit":    "20",
    }
...
def download_snapshot(timestamp: str, original_url: str, session: requests.Session) -> tuple:
    """
...
def strip_wayback_artifacts(html: str) -> str:
    ...
    # Fixed: old regex spanned multiple script blocks. New version uses
    # a negative-lookahead to stay confined to one <script>...</script> block.
    html = re.sub(
...
def analyze_content(text: str) -> list:
    """
...
def process_url(url: str, output_dir: Path, years: int, session: requests.Session) -> dict:
    ...
    if best_findings and best_text:
        ...
        # Wrap non-HTML content in a proper HTML shell so files open
        # readably in a browser regardless of original type.
        raw_ext = os.path.splitext(urllib.parse.urlparse(url).path)[1].lower()
        is_html = raw_ext in (".html", ".htm", "") and best_text.lstrip().startswith("<")
        if is_html:
            save_content = best_text
        else:
            esc_body = best_text.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
            save_content = (
                '<!DOCTYPE html>\n<html lang="en"><head>'
                '<meta charset="UTF-8">'
                ...
                f'Snapshot: <code>{best_snap}</code>&nbsp;|&nbsp;'
                f'<a href="{html_escape(best_arch_url)}" target="_blank">Archive link</a></p>'
                f'<pre>{esc_body}</pre>'
                '</body></html>'
            )
    ...
    # Polite inter-URL delay (once per URL, not once per snapshot).
    # 3 threads x 1.5 s ≈ 1 request/s to archive.org — well within limits.
    time.sleep(1.5)
    return result
...
def main():
    ...
    # Each worker thread gets its own Session — a single shared Session is
    # NOT thread-safe; concurrent threads corrupt urllib3's connection pool.
    _UA = (
    ...
    def worker(url: str) -> dict:
        """Each call owns its own Session — threads never share state."""
        return process_url(url, output_dir, args.years, make_session())
